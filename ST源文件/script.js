...
import { cancelDebouncedMetadataSave, doDailyExtensionUpdatesCheck, extension_settings, initExtensions, loadExtensionSettings, runGenerationInterceptors } from '/api/plugins/cocktail-plus/module?path=%2Fscripts%2Fextensions.js';
...
import { initLocales, t } from '/api/plugins/cocktail-plus/module?path=%2Fscripts%2Fi18n.js';
...
import { initWelcomeScreen, openPermanentAssistantChat, openPermanentAssistantCard, getPermanentAssistantAvatar } from '/api/plugins/cocktail-plus/module?path=%2Fscripts%2Fwelcome-screen.js';
...
import { getSystemMessageByType, initSystemMessages, SAFETY_CHAT, sendSystemMessage, system_message_types, system_messages } from '/api/plugins/cocktail-plus/module?path=%2Fscripts%2Fsystem-messages.js';
...
async function firstLoadInit() {
    console.info('[cp:module-proxy] firstLoadInit patches active', {"version":"0.1.19","patchStartupInit":true,"bootstrapParallel":true,"systemMessagesSettingsParallel":true,"coreDataParallel":true,"loaderViewportParallel":true,"postFirstPaintDeferred":true,"fixViewportFunction":true,"readyCallbackStart":true,"readyFirstLoadCall":true,"bootstrapApplied":true,"systemMessagesSettingsApplied":true,"coreDataApplied":true,"loaderViewportApplied":true,"postFirstPaintDeferredApplied":true,"readyFirstLoadStartedEarlyApplied":false,"firstLoadDiagnosticInserted":true});
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.enter', {"version":"0.1.19","patchStartupInit":true,"bootstrapParallel":true,"systemMessagesSettingsParallel":true,"coreDataParallel":true,"loaderViewportParallel":true,"postFirstPaintDeferred":true,"fixViewportFunction":true,"readyCallbackStart":true,"readyFirstLoadCall":true,"bootstrapApplied":true,"systemMessagesSettingsApplied":true,"coreDataApplied":true,"loaderViewportApplied":true,"postFirstPaintDeferredApplied":true,"readyFirstLoadStartedEarlyApplied":false,"firstLoadDiagnosticInserted":true});
    try {
    ...
    applyBrowserFixes();
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-version-secrets-locales');
    const clientVersionPromise = getClientVersion();
    await initSecrets();
    const secretStatePromise = readSecretState();
    const localesPromise = initLocales();
    await Promise.all([clientVersionPromise, secretStatePromise, localesPromise]);
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-version-secrets-locales');
    globalThis.__cocktailPlusEarlyBridge?.startExtensionPrefetch?.('after-initLocales');
    initChatUtilities();
    ...
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-initPresetManager');
    await initPresetManager();
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-initPresetManager');
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-systemMessages-settings-parallel');
    const systemMessagesPromise = initSystemMessages().finally(() => globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-initSystemMessages'));
    const settingsPromise = getSettings(initLoaderHandle).finally(() => globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-getSettings'));
    await Promise.all([systemMessagesPromise, settingsPromise]);
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-systemMessages-settings-parallel');
    initKeyboard();
    initDynamicStyles();
    initTags();
    initBookmarks();
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-core-data-parallel');
    const userAvatarsPromise = getUserAvatars(true, user_avatar);
    const charactersPromise = getCharacters();
    const backgroundsPromise = getBackgrounds();
    const tokenizersPromise = initTokenizers();
    await Promise.all([userAvatarsPromise, charactersPromise, backgroundsPromise, tokenizersPromise]);
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-core-data-parallel');
    initBackgrounds();
    initAuthorsNote();
    await initPersonas();
    ...
    initServerHistory();
    initWelcomeScreen();
    const cpRunPostFirstPaintInit = async () => {
        globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.post-first-paint-init-start');
        try {
            initSettingsSearch();
            initBulkEdit();
            initReasoning();
            ...
            initItemizedPrompts();
            initAccessibility();
            if (typeof initSwipePicker === 'function') initSwipePicker();
            addDebugFunctions();
            doDailyExtensionUpdatesCheck();
        } finally {
            globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.post-first-paint-init-end');
        }
    };
    (globalThis.requestIdleCallback || function (cb) { return setTimeout(cb, 1); })(() => { void cpRunPostFirstPaintInit(); });
    await eventSource.emit(event_types.APP_INITIALIZED);
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-loader-viewport-parallel');
    const loaderHidePromise = initLoaderHandle.hide();
    await fixViewport();
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.before-app-ready');
    const appReadyPromise = eventSource.emit(event_types.APP_READY);
    await Promise.all([loaderHidePromise, appReadyPromise]);
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('firstLoadInit.after-loader-app-ready-parallel');
}
...
jQuery(async function () {
    globalThis.__cocktailPlusEarlyBridge?.markStartup?.('ready-callback.enter');
    setTimeout(function () {
...
try { console.info('[cp:module-proxy] loaded /script.js', {"version":"0.1.19","patchStartupInit":true,"bootstrapParallel":true,"systemMessagesSettingsParallel":true,"coreDataParallel":true,"loaderViewportParallel":true,"postFirstPaintDeferred":true,"fixViewportFunction":true,"readyCallbackStart":true,"readyFirstLoadCall":true,"bootstrapApplied":true,"systemMessagesSettingsApplied":true,"coreDataApplied":true,"loaderViewportApplied":true,"postFirstPaintDeferredApplied":true,"readyFirstLoadStartedEarlyApplied":false,"firstLoadDiagnosticInserted":true}); } catch (_) {}
